...
void draw()
{
  ...
  for (i=0; i<h; i++) // these pixels are the rows
  {
    for (j=0; j<w; j++) // thise pixels are the columns
    {
      ...
      sum = 0; // start blank
      int colorScale = 128; //int(random(0, 25));
      sum+= int(green(p0)>colorScale) + int(green(p1)>colorScale) + int(green(p2)>colorScale); // top neighbors
      sum+= int(green(p3)>colorScale) + int(green(p5)>colorScale); // left and right neighbors
      sum+= int(green(p6)>colorScale) + int(green(p7)>colorScale) + int(green(p8)>colorScale); // bottom neighbors
      ...
      {
        float r = int((random(0, 128)*w+i)%255);
        float g = int((random(0, 128)*w+i)%255);
        //float b = int((random(0, 128)*w+i)%255);
        c[1-which].pixels[i*w + j] = color(r, 255, 200);
      } else if (center==0 && sum==3) // dead... become alive
      {
        c[1-which].pixels[i*w + j] = color(191, 235, 242);
